...
elseif(is_blog()) { echo 'Latest Posts'; }
...
elseif(is_404()) { echo'Not Found - 404'; } ?></div>
		<?php while ( show_count() < post_count() ) { global $posts,$lolzzc; echo '<div class="section_box2" align="justify"><div class="post_title"><a href="'.content_link().'">'.the_title().'</a></div><div class="text_area">'; echo the_summery(); echo'<div class="publish_date">Posted by: <a href="'.author_link().'">'.the_user().'</a> date: '.the_date().' time: '.the_time().' Category: <a href="'.the_category_link().'">'.the_category().'</a>'; echo'</div></div></div>'; end_post(); } ?>
					<br>
			<div id="pagination"><ul><?php post_navigation(); ?></ul></div>
