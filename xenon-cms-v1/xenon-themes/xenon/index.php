...
elseif(is_home() && !home_page()) { echo 'Latest Posts'; }
...
elseif(is_404()) { echo'Not Found - 404'; } ?></div>
		<?php while ( show_count() < post_count() ) { global $posts,$lolzzc; echo '<div class="section_box2" align="justify"><div class="post_title"><a href="'.content_link().'">'.the_title().'</a></div><div class="text_area">'; if(is_page()) { echo the_content(); } elseif(is_post()) { echo the_content(); } elseif(home_page()) { echo the_content(); }  else { echo the_summery(); } if(!home_page()) { echo'<div class="publish_date">Posted by: <a href="'.author_link().'">'.the_user().'</a> date: '.the_date().' time: '.the_time(); if(!home_page()) { echo' Category: <a href="'.the_category_link().'">'.the_category().'</a>'; }  echo'</div>'; } echo'</div></div>'; end_post(); } ?>
					<br>
